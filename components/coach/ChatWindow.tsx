    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg-card)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '1rem', borderTop: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: '1rem' }}>
                        {SUGGESTED_CHIPS.map(chip => (
                            <button key={chip}
                                ...
                                disabled={isLoading}
                                style={{
                                    padding: '6px 12px', fontSize: '0.75rem', borderRadius: 20,
                                    border: '1px solid var(--border)', background: 'var(--bg-card)',
                                    color: 'var(--text-secondary)', cursor: isLoading ? 'default' : 'pointer',
                                    transition: 'all 0.2s'
                                }}
                                onMouseEnter={(e) => { if (!isLoading) { e.currentTarget.style.color = 'var(--accent)'; e.currentTarget.style.borderColor = 'var(--accent)' } }}
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '0.5rem' }}>
                    <textarea
                        className="input"
                        placeholder={`Ask AI Coach about your ${selectedAcc?.firm_name ?? ''} performance... (Shift+Enter for newline)`}
                        style={{ flex: 1, border: 'none', background: 'transparent', resize: 'none', minHeight: 44, maxHeight: 150, padding: '10px 12px' }}
                        value={input}
                    ...
                    <button
                        ...
                        style={{ height: 44, width: 44, borderRadius: 8, padding: 0, flexShrink: 0, marginBottom: 2 }}
