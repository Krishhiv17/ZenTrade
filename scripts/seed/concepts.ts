...
export const SEED_CONCEPTS: SeedConcept[] = [
  ...
  {
    concept: 'High-Probability Entry Confluence (A+ Setup)',
    category: 'entry_confluence',
    content: `A high-probability ("A+") entry is never a single PD array touched in isolation — it is a stack of confluences pointing the same way. The more of the following that align, the higher the odds. (1) Liquidity first: an obvious high or low is swept — a stop run that takes resting liquidity — BEFORE the setup forms, so the move has fuel and the level's traders are trapped. (2) Displacement: a decisive, expansive move follows the sweep, creates the PD array (an FVG or order block), and shifts market structure (MSS) in the new direction — slow, overlapping candles are not displacement. (3) Overlap of arrays: the entry zone stacks multiple arrays — an FVG sitting inside an order block, or a breaker that also leaves an imbalance — rather than one thin gap. (4) Premium/discount alignment: for longs, the entry is in discount (below equilibrium of the dealing range); for shorts, in premium (above it) — you buy cheap and sell expensive, never chase. (5) Draw-on-liquidity alignment: the trade points toward the higher-timeframe liquidity objective and bias; taking a long while the HTF draw is a sell-side pool below is low quality. (6) Killzone timing: it forms inside London or New York AM, not in dead midday hours. A clean example for a long: price sweeps a session low (liquidity), displaces up and breaks structure leaving an FVG, that FVG overlaps a bullish order block and sits in discount, aligned with an HTF draw to buy-side liquidity above, during the NY AM killzone. Common mistakes: entering on a bare FVG or order-block touch with none of these confluences, ignoring where liquidity rests, and buying in premium (or selling in discount) simply because momentum looks strong.`,
    source: 'internal-paraphrase',
  },
  {
